...
首先，我大概要做的，就是通过切面来增强操作数据库的方法，在插入操作之前为实体的对象赋值，那么我首先要知道实体在插入数据库的方法里面是第几个参数，这里就做了规定，如果要插入数据库的话，是把实体放在第一个参数。然后我还要知道，插入数据库的类型是新增还是修改，这里我是通过注解实现的，插入数据库的方法上面都加了自定义注解，自定义注解的value属性的类型为自定义枚举类型，枚举类型里面只能是“UPDATE”或“INSERT”，那么在插入数据库的方法上面就通过注解标记了插入数据库是更新还是新增。

然后，在切面里面，我就能通过jointpoint获得被增强的方法的参数和注解，获得被增强的方法的注解我就知道了插入数据库的类型，获得被增强方法的参数我就知道了要插入数据库的实体。然后，我就可以通过反射调用实体的set方法为实体的属性赋值。

项目中有实现了`Id`自动填充功能以及`createTime`，`updateTime`的自动填充功能。

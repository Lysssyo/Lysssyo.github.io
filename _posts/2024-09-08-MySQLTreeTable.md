...
```java
    public List<CourseCategoryTreeDto> queryTreeNodes(String id) { // 这里id恒为1，因为1是根结点
        List<CourseCategoryTreeDto> courseCategoryTreeDtos = courseCategoryMapper.selectTreeNodes(id);//查询全表
        ...
        courseCategoryTreeDtos
                ...
                // 检查每一个item
                .forEach(item -> {
                    // 这个if，用于把2级结点 放入结果List（根结点为1级结点）
                    if (item.getParentid().equals(id)) {
                        // 不是所有 courseCategoryTreeDtos 都要放进去结果List中，
                        // 只有 courseCategoryTreeDtos 的ParentId等于id才放，
                        // 剩余的放结果List的每一项的ChildrenTreeNodes
                        categoryTreeDtos.add(item);
                    }
                    // 下面是为了把子节点放入父节点的childrenTreeNodes，
                    // 3级结点放入2级结点的childrenTreeNodes，4级结点放入3级结点的childrenTreeNodes
                    // 找到当前结点的父节点（利用Map）

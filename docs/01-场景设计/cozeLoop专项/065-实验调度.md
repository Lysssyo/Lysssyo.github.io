...
2.  **模式策略 (Strategy Pattern)**:
    *   **`Submit/Append` 模式**: 检查数据集定义的 Turns，对比数据库（指`expt_turn_result_run_log`）。如果发现库里缺记录，则使用 `BatchCreateNX` 批量插入状态为 `Processing` 的 `expt_turn_result_run_log`。

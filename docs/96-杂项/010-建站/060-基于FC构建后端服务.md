...
**解决方案**：引入一个Serverless 后端（阿里云函数计算 FC）**作为“中间人代理”。
...
sequenceDiagram
    ...
    participant FE as 前端 (PrivateVault.vue)
    participant FC as 阿里云函数计算 (Node.js)
    participant GH as GitHub API

    Note over User, FE: 阶段一：解锁 (Action: list)
    User->>FE: 输入访问密码并点击解锁
    FE->>FC: POST / (body: {password, action: 'list'})
    
    rect rgb(240, 248, 255)
        Note right of FE: FC 内部处理
        FC->>FC: 1. 解析 Event 获取 Body
        FC->>FC: 2. 校验 password == process.env.ACCESS_PASSWORD
        FC->>GH: 3. 携带 GITHUB_TOKEN 请求 /git/trees/main
    end
    
    GH-->>FC: 返回文件树 JSON
    FC-->>FE: 返回过滤后的 .md 文件列表
    FE-->>User: 渲染左侧目录树

    Note over User, FE: 阶段二：读取 (Action: content)
    User->>FE: 点击某个文件
    FE->>FC: POST / (body: {password, action: 'content', path: 'xxx.md'})
    
    rect rgb(240, 248, 255)
        FC->>FC: 校验密码
        FC->>GH: 携带 Token 请求 /contents/xxx.md
    end
    
    GH-->>FC: 返回 { content: "Base64串", ... }
    FC-->>FE: 透传 GitHub 响应
    FE->>FE: Base64 解码 -> UTF-8 -> Markdown 渲染
    FE-->>User: 展示渲染后的 HTML
```

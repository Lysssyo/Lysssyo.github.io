...
date modified: 2026-02-10 20:47:47
---
...
> [!IMPORTANT] 核心洞察
> **在使用 Quorum Queues 时，发布者确认是客户端与共识系统交互的唯一桥梁**。
> 只有收到 Confirm，客户端才能确信消息不仅到达了 Leader，而且已经被安全地复制到了 Follower 节点，具备了容灾能力。
...
> 参考：[Raft 一致性协议](../../05-分布式/020-Raft%20一致性协议.md)、 [3.2.3 仲裁队列](010%20-%20RabbitMQ%20深度剖析.md#3.2.3%20仲裁队列)
...
RabbitMQ 的消息确认机制决定了消息何时从队列中被永久删除。主要分为**自动确认**和**手动确认**两种模式。
...
RabbitMQ **不使用超时机制**（Timeout）来判断消费者是否故障，而是完全依赖 **TCP 连接状态**。这是其“推模式（Push）”的核心逻辑：

1. **正常情况**：Broker 盯着 TCP 连接，只要连接不断，它就认为消费者还在处理，允许消息长时间处于 `Unacked`。
2. **故障恢复**：一旦消费者进程崩溃导致 TCP 连接断开（Socket 关闭），Broker 立即感知，将该连接持有的所有 `Unacked` 消息重新变更为 `Ready`，重回队列，**并重新投递给其他存活的消费者**。（这是 Broker 的内部动作，Smart Broker）
...
> [!CAUTION] **警惕“毒消息” (Poison Message) 死循环**

- **首选 Nack**：尽量使用 `channel.basicNack`，因为它比 `basic.reject` 更灵活，支持批量操作。
- **慎用 `requeue=true`**：
    - **风险**：如果是**代码逻辑错误**（如空指针异常、除以零），消息处理失败后如果无限重回队列（`requeue=true`），会导致“投递 -> 报错 -> 回队 -> 投递”的无限死循环。
    - **正确做法**：对于无法恢复的业务异常，应使用 `basic.nack(requeue=false)`，并配置 **死信队列 (DLX)**。让问题消息进入死信队列“冷静一下”，后续由人工介入或独立的服务进行补偿处理。
...
RocketMQ 的消费端可靠性依赖于严格的 Offset 提交机制，以及由 **Consumer SDK 主导** 的重试逻辑。

#### 4.3.1 消费重试与死信

RocketMQ 的重试机制并非像 RabbitMQ 那样由服务端监听 NACK 信号触发，而是由 **Consumer SDK（客户端）** 在本地捕获异常后，主动发起 RPC 请求实现的。

> [!TIP]
> 所以不同语言的实现是有差别的

**1. 业务处理失败（主动重试 - SDK 驱动）** 当消费逻辑返回 `RECONSUME_LATER` 或抛出未捕获异常时，Broker 并不会自动感知。

- **机制：** Consumer SDK 捕获到失败状态后，会构建一个 `CONSUMER_SEND_MSG_BACK` 请求，将该消息**重新发送**回 Broker。
- **存储：** Broker 收到请求后，将消息写入特殊的 **重试队列**（Topic 为 `%RETRY%消费者组`），并根据重试次数设定延时等级。
- **注意：** 如果 Consumer 无法将消息发回 Broker（如网络阻断），SDK 会退化为本地内存重试。
    

**2. 消费者宕机或断网（被动重发 - Rebalance）** 如果 Consumer 挂掉，SDK 无法发送任何信号，Broker 也 **没有** 单条消息的超时机制。

- **机制：** Broker 通过心跳检测发现连接断开，触发 **Rebalance（重平衡）**。
- **结果：** 该队列被分配给同组的其他 Consumer，新的 Consumer 从服务端记录的 **最后一次提交的 Offset** 开始拉取。这会导致未提交 Offset 的消息被“重复消费”，而非标准意义上的“重试”。
    

**3. 重试策略与死信**

- **阶梯重试：** 消息在 `%RETRY%` 队列中流转。Broker 按照 18 个延时级别（1s, 5s, 10s, 30s... 2h）逐步增加再次投递的间隔。
- **死信队列（DLQ）：** 当重试次数超过上限（默认 16 次）后，SDK 不再发回重试队列，而是发送到 **死信队列（DLQ，Topic 为 `%DLQ%消费者组`）**，此时消息不再投递，需人工干预。

**4. 核心原理：Broker 实现阶梯重试

Broker 实现“阶梯重试”的机制，实际上复用了 RocketMQ 的 **定时消息（Delay Message）** 功能。

当 Broker 收到 SDK 发来的 `CONSUMER_SEND_MSG_BACK` 请求时，它做了一系列“神操作”：

1. **偷换 Topic**：Broker 发现这是一个重试请求（带有 Delay Level），它**不会**直接把消息放回 `%RETRY%` 队列（否则立马又被消费了）。它会把 Topic 改成一个内部系统 Topic：`SCHEDULE_TOPIC_XXXX`。
2. **存入“停尸房”**：消息被写入这个系统 Topic，暂存起来。
3. **Broker 内部倒计时**：Broker 内部有一个专门的后台线程（`ScheduleMessageService`），它像一个定时器一样，不断扫描 `SCHEDULE_TOPIC_XXXX`。
4. **还原**：当倒计时结束（比如 10秒到了），Broker 会自动把这条消息拿出来，**把 Topic 改回** `%RETRY%消费者组`，这时刻消费者才能再次看到并拉取到这条消息（Consumer SDK 在启动时，偷偷“自动订阅”了重试 Topic）。
...
> [!TIP] 复制机制
> 所以，与RocketMQ 的复制机制依赖配置 `SYNC_MASTER` 或者 `ASYNC_MASTER` 不同。Kafka 的复制策略由生产者指定
...
**原生 Kafka 实际上“没有”类似 RocketMQ 那样的消费端失败重试机制。**

Kafka 的设计哲学是 **“流处理（Streaming）”** 而非 **“业务消息队列（Messaging）”**。它追求极致的吞吐量，因此核心非常精简，不内置任何“消费状态追踪”、“单条消息重试”、“死信队列”等复杂的业务逻辑。

在 Kafka 中，所谓的“重试”，完全是**由开发者（或上层框架，如 Spring Kafka）在应用层手写代码实现的**。

如果你使用原生 Kafka 客户端（`kafka-clients`），当消费一条消息抛出异常时，只有两个残酷的选择：

1. **提交 Offset（丢消息）：** `catch` 异常后记录日志，然后提交 Offset。    
    - **后果：** 消息丢失，永远不会重试。
2. **不提交 Offset（死循环）：** 抛出异常，不提交当前 Offset。
    - **后果：** 下一次 `poll()` 拉取时，Consumer 依然会拉到**这一批**消息（包括那条失败的）。如果代码逻辑没变，它会再次抛异常 -> 再次拉取。
    - **现象：** **“Head-of-Line Blocking”（队头阻塞）**。一条坏消息卡住了整个 Partition，后续的好消息全部无法处理。

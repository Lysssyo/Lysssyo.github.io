...
date modified: 2026-02-11 10:52:11
---
...
- **特点**：
	- 日志复制与快照：Quorum Queue 的核心是一个仅追加的日志文件。所有的队列操作（入队、出队、Ack）都是日志条目。为了防止日志无限增长，RabbitMQ 会定期执行快照并压缩日志。
	- 多数派写入
	- 确定性选主

> [!TIP] Raft Log
> 在 Raft 算法中，为了保证集群一致性，Leader 必须把操作指令复制给 Follower。这些指令就是 **Raft Log**。 每一个 Log 条目包含：
> - **Index**：序号（第 1 条，第 2 条...）。
> - **Term**：任期号（这是哪一代 Leader 发布的）。
> - **Command**：具体指令（例如 `SET x = 1` 或 `Enqueue Msg A`）。

#### 3.2.3.2 写入流程

Quorum Queues 的核心在于“多数派写入”机制。以下图表展示了一条消息如何才算“写入成功”。
...
1. **初始写入**：客户端发送消息给 Leader，Leader 的 Raft 模块把消息封装为一个 Log Entry（包含 Term, Index, Data），然后把这个 Log Entry 写入底层的 WAL 文件（此时还在 Page Cache），然后调用 `fsync` 刷屏
2. **日志复制**：Leader 并行将日志条目发送给 Follower 节点。
3. **多数派共识（核心保障）**：Leader 必须等待集群中 **(N/2)+1** 个节点（例如 3 个节点中的 2 个）反馈“写入成功”。
4. **提交确认**：只有达成多数派共识，Leader 才会向客户端返回 Ack。**这意味着一旦你收到 Ack，即使 Leader 下一秒宕机，数据也已经在其他节点安全存储。**

> [!TIP]
> 在工程实现中，**Step 1 (Leader落盘)** 和 **Step 2 (复制给Follower)** 实际上是**并行**触发的。Leader 不会因为等待本地磁盘 IO 而阻塞网络发送，这样能显著降低整体的写入延迟。
> 但无论如何，**“不凑齐多数派落盘确认，绝不给客户端返回 Ack”** 这一铁律是不变的。

#### 3.2.3.3 三大安全保障机制

Quorum Queues 利用 Raft 算法解决了传统队列的痛点：

**“写完不丢”保障 (Write Safety)**

- **机制**：**多数派写入 + 强制持久化**。
- **对比**：旧版镜像队列可能在主节点写入内存但未同步给从节点时宕机，导致消息丢失。Quorum Queues 强制要求“**多数节点已落盘**，数据具备容灾能力”后才视为成功。
    

**“选主不丢”保障 (Election Safety)**

- **机制**：**确定性选主 (Log Completeness)**。
- **场景**：假设 Leader A 宕机，Node B 拥有最新数据，Node C 数据陈旧（网络快）。
- **逻辑**：Raft 铁律规定 **“选票只投给日志比我新（或一样新）的人”**。因此，数据陈旧的 C 无法获得多数票，只有拥有所有已提交日志的 B 能当选。
- **结果**：新 Leader 一定包含所有已 Ack 的消息，历史记录绝不会被篡改或丢失。
    

**“死而复生不错乱”保障 (Log Matching)**

- **机制**：**日志截断与快照 (Snapshotting)**。
- **场景**：旧 Leader 恢复并重新加入集群。
- **逻辑**：它必须无条件服从新 Leader。新 Leader 会指示其 **截断 (Truncate)** 冲突的脏日志，并追加最新日志。如果落后太多，Leader 会直接发送**状态快照 (Snapshot)** 让其瞬间对齐状态。

#### 3.2.3.4 性能权衡与适用场景

| **维度**               | **特性描述**                                              |
| -------------------- | ----------------------------------------------------- |
| **延迟 (Latency)**     | **较高**。因为必须等待**多数节点已落盘**。比纯内存的经典队列要慢。                 |
| **吞吐量 (Throughput)** | **极其稳定**。在高并发下，它表现为一条平滑的直线。不会像经典队列那样因为内存吃紧而导致性能抖动或崩溃。 |
| **资源消耗**             | **磁盘 I/O 密集型**。内存主要用于缓存热数据，冷数据会迅速落盘。                  |

**总结**：如果你需要极致的低延迟（毫秒级），Quorum Queues 可能不是首选；但如果你需要**金融级的数据可靠性**以及**高并发下的系统稳定性**，它是目前的最佳选择。

### 3.2.4 刷盘策略

RabbitMQ 没有像 RocketMQ 那样有一个全局的 `flushDiskType` 开关来强制所有消息同步或异步刷盘。

它把决定权交给了 **“消息的发送者”** 和 **“队列的定义者”**。只有满足以下**两个条件**，消息才会被写到硬盘：

1. **队列是持久化的 (Durable Queue)：**
    - 在声明队列时设置 `durable = true`。如果不设置，RabbitMQ 重启后，这个队列元数据本身就会消失。
2. **消息是持久化的 (Persistent Message)：**
    - 发送消息时，设置 `delivery_mode = 2`。
    - 如果不设置（默认是 1），消息只存在于内存中。一旦 Broker 重启或崩溃，内存里的消息直接丢失。

**刷盘的时机 (fsync)**

即便你设置了“持久化消息”，RabbitMQ 也**不是**每收到一条就立马调用 `fsync` 落盘（那样太慢了）。

- **缓冲机制：** RabbitMQ 有一个内部的 **Persistence Layer**。它会把准备落盘的消息先缓存起来。
- **批量刷盘：** 它会在几百毫秒或者积累了一定数量后，批量调用 `fsync`。
- **这就意味着：** 如果没有用“发布确认（Publisher Confirm）”机制，在 Broker 断电的瞬间，哪怕你发的是持久化消息，依然可能丢失最近几百毫秒的数据。

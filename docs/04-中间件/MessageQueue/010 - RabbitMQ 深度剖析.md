...
date modified: 2026-02-11 11:14:28
---
...
**RabbitMQ 采用推模式（Push）将消息投递给消费者，消费者竞争消费**。
...
### 3.2.3 仲裁队列 (Quorum Queues)

#### 3.2.3.1 核心定义与设计哲学

Quorum Queues 是基于 **Raft 一致性协议** 构建的队列类型。它的设计哲学只有一条：**数据绝对不能丢**。它**通过牺牲部分延迟性能，换取了极致的数据一致性和系统的稳定性**。

- **定位**：替代旧版“镜像队列”的下一代高可用方案。
- **存储机制**：
    - **Raft Log + WAL**：核心是一个仅追加（Append-Only）的日志文件。
    - **强制持久化**：不支持纯内存（Transient）模式。数据必须先写进 **WAL (Write-Ahead Log)**。
    - **fsync (强制刷盘)**：为了防止断电丢数据，每写一批数据都会调用 OS 的 `fsync` 指令，强制硬件将数据写入磁盘介质（这是主要的性能开销来源）。
- **特点**：
	- **日志复制与快照**：所有的队列操作（入队、出队、Ack）都是日志条目。为了防止日志无限增长，RabbitMQ 会定期执行快照并压缩日志。
	- **多数派写入**：不凑齐多数派落盘确认，绝不给客户端返回 Ack。
	- **确定性选主**：只有日志最新的节点才有资格当选 Leader。

> [!TIP] Raft Log
> 在 Raft 算法中，为了保证集群一致性，Leader 必须把操作指令复制给 Follower。这些指令就是 **Raft Log**。 每一个 Log 条目包含：**Index**（序号）、**Term**（任期号）以及 **Command**（具体指令，如 Enqueue/Ack）。

#### 3.2.3.2 复制流程

Quorum Queues 的核心在于“多数派写入”机制。

![Gemini_Generated_Image_k1cnlrk1cnlrk1cn.png](https://keith-knowledge-base.oss-accelerate.aliyuncs.com/Gemini_Generated_Image_k1cnlrk1cnlrk1cn.png)

**流程解析：**

1. **日志封装**：客户端发送消息给 Leader，Leader 的 Raft 模块把消息封装为一个 Log Entry（包含 Term, Index, Data），然后把这个 Log Entry 写入底层的 WAL 文件（此时还在 Page Cache），然后调用 `fsync` 刷屏
2. **并行复制**：Leader 将日志发送给所有 Follower。
3. **共识达成**：等待集群中 **(N/2)+1** 个节点反馈写入成功。
4. **提交确认**：达成共识后，Leader 向客户端返回 Ack。

**关键特性：ACK 也是一次写操作 (ACK is a Write)**

这是仲裁队列性能与经典队列最大的区别：

- 在经典队列中，ACK 仅是内存标记删除。
- 在仲裁队列中，**“消费确认 (ACK)”被视为一次状态变更的操作**，必须写入 Raft 日志并同步至多数派节点。因此，仲裁队列的 ACK 吞吐量天然低于经典队列。

#### 3.2.3.3 运行期行为：Leader 驱动与非阻塞同步

- **Leader 驱动消费**：仲裁队列的消费者必须从 Leader 获取消息。Leader 会按照轮询或 QoS 规则将消息分发给各消费者。
- **非阻塞式同步**：Raft 算法天然支持日志追赶。当一个节点宕机后重新加入时，它会自动从 Leader 处拉取缺失的日志。整个过程**不会阻塞队列的读写操作**，彻底解决了镜像队列在同步期间全量锁定的痛点。

#### 3.2.3.4 三大安全保障机制

Quorum Queues 利用 Raft 算法解决了分布式系统中的共识问题：
- **“写完不丢”保障 (Write Safety)**：多数派已落盘，数据具备容灾能力。
- **“选主不丢”保障 (Election Safety)**：选票只投给日志比我新（或一样新）的人。新 Leader 一定包含所有已提交的消息。
- **“死而复生不错乱”保障 (Log Matching)**：旧 Leader 恢复后必须服从新 Leader，通过截断冲突日志和状态快照（Snapshot）快速对齐。

#### 3.2.3.5 启用仲裁队列

RabbitMQ 不会自动猜测你想用哪种队列。如果你在声明队列时不指定类型，它默认会创建一个 **Classic Queue（经典队列）**（这是为了兼容旧代码的历史包袱）。

如果你想用现代的、不丢数据的 **Quorum Queue（仲裁队列）**，你必须在声明队列时**显式指定**。

例如：在代码中声明 (以 Java/Spring 为例)

```java
// Spring AMQP 写法
@Bean
public Queue orderQueue() {
return QueueBuilder.durable("order.payment.q")
        .withArgument("x-queue-type", "quorum") // 在声明队列时，放入一个参数 `x-queue-type`，值为 `quorum`。
        .withArgument("x-quorum-initial-group-size", 3) // 显式指定：我要3个副本组成的组
        .build();
}
...
| **维度**               | **特性描述**                                              |
| -------------------- | ----------------------------------------------------- |
| **延迟 (Latency)**     | **较高**。由于必须等待多数派落盘，比纯内存经典队列慢。                 |
| **吞吐量 (Throughput)** | **极其稳定**。不会因内存积压导致 Page Out 带来的性能剧烈波动。 |
| **资源消耗**             | **磁盘 I/O 密集型**。对磁盘 IOPS 敏感。                  |

**总结**：如果你需要极致的低延迟，经典队列更优；但如果你需要**金融级的数据可靠性**及高并发下的稳定性，仲裁队列是唯一选择。

### 3.2.4 刷盘策略：经典队列 vs 仲裁队列

RabbitMQ 的刷盘策略取决于你使用的**队列类型**。这是决定数据可靠性的分水岭。

#### 1. 经典队列 (Classic Queues) —— "异步刷盘"

依赖操作系统的 Page Cache，**不是**强一致性的。

- **触发条件**：
    - **队列持久化** (`durable = true`)
    - **消息持久化** (`delivery_mode = 2`)
- **特点”**：
    - 即便设置了持久化，RabbitMQ 收到消息后，通常**先写内存/缓存**，然后立刻返回 `Ack`。
    - **异步落盘**：它会在几百毫秒后，或积攒了一定数据量后，才调用 `fsync` 真正写进物理磁盘。
    - **后果**：如果在 `Ack` 之后、`fsync` 之前断电，**数据会丢失**，但发送方以为成功了。

> [!TIP] 经典队列下不一定会刷盘
> 如果你发送消息时设置 `delivery_mode = 1`（非持久化），或者队列本身是 `durable = false`，RabbitMQ 会把消息**纯放在内存里**
> 结果：它**根本就不打算刷盘**。只有当内存快爆了（Memory Pressure），它才会为了腾空间把消息“换页（Page Out）”到磁盘上。否则，重启即丢失
> 

#### 2. 仲裁队列 (Quorum Queues) —— "强制同步刷盘 (WAL)"

- **强制持久化**：
    - Quorum Queues 设计上就是为了数据安全，它**忽略**非持久化标志（它总是把消息视为持久化的 Raft 日志条目）。
- **安全的“顺序”**：
    - **先落盘，后 Ack**：只有当消息被写入 WAL 文件并执行了 `fsync`（强制刷盘）**之后**，且大多数节点都完成了这一步，Leader 才会给客户端返回 `Ack`。
- **Group Commit (组提交)**：
    - 为了缓解 `fsync` 的性能损耗，它不会每条消息都刷一次盘，而是将并发到来的多条消息合并成一个 Batch，一次性 `fsync`。
    - **关键点**：虽然是批量刷盘，但**客户端的 Ack 会一直被阻塞（Block）**，直到这个 Batch 真正落盘成功。**绝不会出现“先 Ack 后落盘”的情况。**
...
为了解决镜像队列在强一致性和运维复杂性上的痛点，RabbitMQ 3.8 引入了**仲裁队列**（Quorum Queues）。这一演进标志着 RabbitMQ 从传统的异步链式复制转向了基于 Raft 的强一致性分布式协调模型。

- **解决痛点**：仲裁队列天然免疫网络分区下的“脑裂”问题，并消除了镜像队列在节点加入时导致的全量阻塞同步（即所谓的“惊群效应”）。
- **非阻塞同步**：依托 Raft 协议，新加入的节点可以通过日志追赶实现数据对齐，而无需暂停现有的读写服务。
- **架构转型**：随着元数据管理也向基于 Raft 的 Khepri 迁移（见 4.3），RabbitMQ 正逐步演变为一个全栈具备确定性故障恢复能力的系统。

> [!INFO] 技术细节参考
> 关于仲裁队列的 Raft 日志复制细节、写入确认流程、ACK 成本分析以及 Leader 驱动的消费模式，请参阅 [3.2.3 仲裁队列 (Quorum Queues)](010%20-%20RabbitMQ%20深度剖析.md#3.2.3%20仲裁队列%20(Quorum%20Queues))

...
这是 RabbitMQ 服务端的一个功能。默认情况下，生产者发送消息是“发后即忘”的。开启 Confirm 模式后，Broker 会给生产者一个“回执”。
...
RabbitMQ Broker 不负责重试，它只负责收发。如果网络抖动、Broker 拒绝（Nack）或超时未响应，生产者需要自己决定是否重发。

通常有两种实现方式：

1. **本地重试（Spring AMQP 默认支持）**：在内存中重试，阻塞当前线程。当网络不稳定的时候，利用重试机制可以有效提高消息发送的成功率。不过SpringAMQP提供的重试机制是**阻塞式**的重试，也就是说多次重试等待的过程中，当前线程是被阻塞的。（**关键策略：指数退避 (Exponential Backoff)**）
2. **异步重试**：将失败消息存入本地数据库或缓存，由后台线程扫描重发。
...
Spring本地重试达到最大次数后，默认会丢弃消息。Spring允许自定义重试耗尽后的策略，由`MessageRecovery`接口定义：

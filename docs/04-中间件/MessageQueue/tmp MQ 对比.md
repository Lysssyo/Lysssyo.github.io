...
| **故障类型**               | **RabbitMQ (Quorum + SAC)**                                 | **Kafka (Idempotent)**                                           | **RocketMQ (Orderly)**                                            |
| ---------------------- | ----------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------- |
| **消费者宕机**              | **自动切换**<br><br>  <br><br>SAC秒级切换。未Ack消息重回队头，需幂等处理。         | **Rebalance**<br><br>  <br><br>几十秒级停顿。从Last Commit Offset接续，需幂等。 | **锁过期/切换**<br><br>  <br><br>最多60秒延迟（等待锁过期）。从Last Commit Offset接续。 |
| **Broker宕机**           | **Raft选举**<br><br>  <br><br>Quorum Queue保证日志一致性，切换透明。       | **ISR选举**<br><br>  <br><br>保证高水位线数据一致，unclean=false防止乱序。         | **Master-Slave切换**<br><br>  <br><br>DLedger模式下Raft选主，保证数据一致。      |
| **毒丸消息 (Poison Pill)** | **丢弃/DLQ**<br><br>  <br><br>达到delivery-limit后移出，打破顺序，恢复可用性。 | **阻塞**<br><br>  <br><br>通常需人工介入提交Offset跳过，或死循环重试。                | **阻塞->DLQ**<br><br>  <br><br>本地重试N次后移入DLQ，兼顾了严格性与最终恢复。            |
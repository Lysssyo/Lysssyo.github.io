...
Kafka 客户端（Producer 和 Consumer）并非简单的 socket 封装，而是复杂的分布式系统组件，内部维护了大量的状态管理、缓冲逻辑和协议交互。

### 4.1 生产者：缓冲池与发送线程模型

Kafka Producer 是线程安全的，其内部包含一个主线程和一个后台 I/O 线程（Sender Thread）。

#### 4.1.1 Record Accumulator
...
- 消费者首先检查自己的 **内部缓存（CompletedFetches）**。为了提高效率，消费者通常会预取（Prefetch）数据。如果缓存中已经有之前拉取到的数据，它会直接返回给应用层，而不需要立刻发起网络请求。
...
作为分布式系统，Kafka 必须在节点故障、网络分区等不确定性中保证数据的一致性（Consistency）和持久性（Durability）。

### 5.1 副本复制协议：ISR 与 HW/LEO

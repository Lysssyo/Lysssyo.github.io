...
date modified: 2026-02-10 23:58:15
---
# Apache Kafka 深度剖析
...
> [!tip] Kafka 的刷盘策略
> Kafka **默认不支持（也不建议配置）**“同步刷盘”。它的高可靠性完全依赖于 **多副本机制（Replication）**，而不是单机的硬盘持久化
...
在 Kafka 中，当你看到 Broker 确认收到消息时，**消息可能根本还没写到硬盘上**。

- **默认策略（Async Flush）：** Kafka 收到消息后，只是把它写到了操作系统的 **PageCache（页缓存）** 中，然后就认为写入成功了。
    - Kafka **不主动** 调用 `fsync()`。
    - 它完全依赖 Linux 操作系统的后台线程（`pdflush` 或 `vm.dirty_writeback_centisecs`）来决定什么时候把缓存刷入物理硬盘。
- **为什么这么狂？**
    - **性能：** 内存写入速度比硬盘快几个数量级（纳秒级 vs 毫秒级）。这是 Kafka 吞吐量能达到百万级的核心原因。
    - **理念：** Kafka 认为，与其花时间等待单机硬盘落盘，不如把这个时间用来把数据复制到另一台机器的内存里。**“多机内存冗余”比“单机硬盘冗余”更安全且更快。**


而且，Kafka 的设计中有一个反直觉的决策：不在 JVM 堆内维护数据缓存。相反，它**完全依赖操作系统的 Page Cache** 。

...
**标准的 BKD-Tree 论文/理论模型**：[010-从 KD-Tree 到 BKD-Tree](https://www.google.com/search?q=010-%E4%BB%8E%2520KD-Tree%2520%E5%88%B0%2520BKD-Tree.md)

### 5.1 核心架构

在学术界理论与 Lucene 的工程实现之间，存在一个重要的映射关系：

**1. 森林 (Forest) 的实现差异**

- **学术界 BKD**：一个索引 = 内存 Buffer + 磁盘上的 $T_0, T_1, T_2$（一组树）。
    
- **Lucene 实现**：Lucene 利用自身的 **Segment（段）架构** 来充当“森林”。
    
    - 一个 ES Shard = 内存 Indexing Buffer + 磁盘上的 Segment 0, Segment 1, Segment 2...
        
    - **单文件单树**：Lucene 的一个 `.dim` 文件（对应一个 Segment）内部只是一棵单一的 K-D Tree。
        
    - **合并即归并**：当 ES 进行 Segment Merge 时，实际上就是在做 BKD 论文里提到的“将两棵小树合并成一棵大树”的操作。
...
> [!tip]
> 
> 既然每个 Segment 内部只是一棵 Packed K-D Tree，为什么 Lucene 仍称其为 BKD？
> 
> 因为它完整保留了 BKD 论文中关于**磁盘布局**的核心优化技术：
> 
...
>     
> 
> **结论**：Lucene 用 **Segment 体系**实现了 BKD 的动态特性（森林），用 **`.dim` 文件**实现了 BKD 的静态特性（分块树）。

### 5.2 物理存储结构 (.dii 与 .dim)

Lucene 的 BKD 实现将数据分为两部分存储：内存索引（导航）与磁盘数据（存储）。
...
- **内容**：包含根节点、中间节点以及指向磁盘的末梢节点。
    
- **关联方式**：其中末梢节点存的是指向磁盘文件的**文件偏移量 (File Pointer)**，即块的起始字节位置。
...
- **结构**：没有任何复杂的树形结构，就是**一个个连续排列的 Leaf Block（叶子块）**。
    
- **容量**：Lucene 默认一个叶子块（Leaf Block）存储 **1024 个点**（这比传统数据库的页要小）。
    
- **作用**：真正存储数据的地方。
    

### 5.3 叶子节点 (Leaf Blocks) 微观解剖

Leaf Block 内部并非简单的表格，而是**高度压缩**的结构。

#### 5.3.1 逻辑结构

逻辑上包含两部分：**数值 (Values)** 和 **文档ID (DocIDs)**。
...
采用了 **列式存储 (Columnar)** 思想，DocID 与数值分离存储并压缩。
...
1. **DocID 的差值压缩 (Deltas)**
    
    - Lucene 会尽量紧凑存储 DocID。
        
    - **原理**：存基准值和差值。例如 `[100, 101, 105]` 存为 `100, 1, 4`。
        
    - **效果**：用极少的 bit 就能存下 DocID。
        
2. **数值的公共前缀压缩 (Common Prefix)**
    
    - 这是 BKD 树省空间的精髓。在一个 Block 里，坐标往往非常接近。
        
    - **原理**：提取公共前缀，只存后缀。
        
    - **示例**：点 A (`120.15678`) 和 点 B (`120.15679`)，提取前缀 `120.1567`，只存 `8` 和 `9`。
        
    - **效果**：原本 8 字节的 `double` 可能只需要 1 个字节。
        

### 5.4 只有 DocID：如何关联文档？

BKD-Tree (即 `.dim` / `.dii`) 的工作到 **返回 DocID** 这一步就**彻底结束**了。它不负责告诉你文档的内容是什么。**DocID 是连接 Lucene 其他组件（正向索引、列式存储）的唯一钥匙。**
...
我们知道，倒排索引、BKD树，都是拿到 `docID` 就收工了，那么拿到 `docID` 之后发生了什么呢？

简单来说，拿到 DocID 只是完成了第一步（**Query Phase**），接下来会进入第二步（**Fetch Phase**），也就是“去仓库取货”的阶段。
...
### 7.1 宏观流程：Query Then Fetch

Elasticsearch 的搜索默认是 `query_then_fetch` 策略。

1. **Query Phase (查询阶段)**：
    
    - 在各个分片（Shard）上，利用 **BKD-Tree**（针对 `evaluator_score`）快速找到满足 `gt: 0.5` 的所有 **DocID**（例如 `[10, 25, 42]`）。
        
    - 因为查询在 `filter` 中，不需要计算评分，所以速度极快。
        
    - **此时，还没有读取文档的具体内容。** 分片只把 **DocID 列表**（以及排序信息，默认是 `_doc` 顺序）返回给**协调节点 (Coordinating Node)**。
...
现在我们聚焦到**Fetch Phase**，看看分片在磁盘上是如何拿着 DocID 找到数据的。这里涉及两个核心文件：**正向文件 (Stored Fields)**。

- **`.fdx` (Field Index)**：索引文件，像书的目录。
    
- **`.fdt` (Field Data)**：数据文件，像书的正文（存了压缩后的 JSON）。

#### 步骤 A：查目录 (.fdx)

Lucene 拿到 `DocID = 10`。它首先去读 `.fdx` 文件。 这个文件非常小，存的是：`DocID -> 物理文件位置 (Offset)` 的映射。 它会告诉 Lucene：“DocID 10 的数据存储在 `.fdt` 文件的第 **4096** 字节处，属于第 **2** 个压缩块 (Chunk)。”

#### 步骤 B：读数据块 (.fdt)

这是开销最大的一步。 Lucene 的 `_source` 字段是**行式存储 (Row-oriented)** 的。为了节省空间，它不是一条条存，而是把每 **16KB**（默认）的数据或者是 **128篇** 文档打包成一个 **Chunk（块）**，然后用 LZ4 或 DEFLATE 算法压缩。

- **动作**：Lucene 跳转到 `.fdt` 的第 4096 字节，**将包含 DocID 10 的整个 Chunk（可能包含 Doc 1 到 Doc 128）全部读取并解压到内存中**。
    
- **代价**：即使你只查 1 个文档，ES 也必须解压它所在的整个块。这就叫“读放大”。
    
#### 步骤 C：定位与解析 (Parsing)

在解压后的数据块中，Lucene 快速定位到 DocID 10 的具体数据区。 此时拿到的是一个完整的、原始的 JSON 字符串（也就是你写入时的完整 `_source`）。

#### 步骤 D：Source Filtering
...
1. ES 解析那个完整的 JSON 对象。
    
2. 提取 `item_id` 和 `evaluator_score` 字段。
    
3. **丢弃** JSON 中的其他所有字段（比如 `user_id`, `timestamp`, `description` 等）。
    
4. 重新封装成一个新的、小的 JSON 对象。
    
#### 步骤 E：返回

分片将这个过滤后的小 JSON 返回给协调节点，协调节点拼装最终结果返回给你。

### 7.3 性能隐患与优化

针对这个查询，有几个性能细节值得注意：

#### 7.3.1 `_source` 过滤不是“列式读取”

**误区**：很多人以为写了 `"_source": ["key"]`，ES 就只从磁盘读这一个字段。 **真相**：**不是的。** ES 依然是从磁盘读取并解压了**完整的**文档 JSON，只是在最后返回给你之前，在内存里把不要的字段扔掉了。网络传输省了，但**磁盘 IO 和解压 CPU 开销一点没少**。

#### 7.3.2 另一种选择：DocValues

如果 `item_id` 和 `evaluator_score` 都是简单类型（keyword, numeric），且开启了 `doc_values`（默认开启）。
...
GET /expt_turn_result_filter/_search
{
  "docvalue_fields": ["item_id", "evaluator_score"], 
  "_source": false,
  "query": { ... }
}
```
...
- 这种写法拿到 DocID 后，**不会去读 `.fdx` 和 `.fdt` (Stored Fields)**。
    
- 它会去读 **`.dvd` (DocValues, 列式存储)**。
    
- **优势**：列式存储支持“只读特定列”。它不需要解压整个 JSON，只需要从磁盘读取 `item_id` 和 `evaluator_score` 这两列的数据。
    
- **场景**：如果你只需要取很少的几个字段，且不需要原始 JSON 结构，`docvalue_fields` 通常比 `_source` 过滤快得多（更少的 IO，更少的解压）。
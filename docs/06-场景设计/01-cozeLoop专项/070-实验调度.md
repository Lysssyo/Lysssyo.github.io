...
date modified: 2026-02-26 16:20:57
---
# CozeLoop 实验调度机制深入解析
...
2.  **`RunExperiment`**:
    ...
    *   **抢占分布式锁 (`manager.LogRun`)**:
        *   尝试获取 Redis 锁 `expt_run_mutex_lock:{ExptID}`。
        *   **设计意图**: 这是一个**实验级长效锁**。它在 API 启动时加锁，直到整个实验彻底结束（Success/Fail）后才由调度器释放。这确保了同一个实验在同一时间只能有一个运行中的批次。
        *   **关键点**: 如果抢锁失败，直接报错 `ExperimentRunningExistedCode`。
...
3.  **`HandleEventLock` (分布式锁)**:
    *   **Mutex**: 抢占 `expt_run_exec_lock:{ExptID}:{RunID}`。
    *   **设计意图**: 这是一个**运行级短效锁**。调度器在每轮循环开始时加锁，通过 `defer unlock()` 在本轮结束时释放。这确保了在多实例部署下，同一个 RunID 只会被一个调度器实例处理。
...
针对 `toSubmit` 列表：
1.  **一致性策略 (MQ Before DB)**:
    系统采用了“先发 MQ，后写状态”的策略，结合分布式锁实现最终一致性：
    *   **步骤 1: 发送 MQ**: 调用 `Publisher` 批量下发 `ExptItemEvalEvent`。
    *   **步骤 2: 更新 DB**: 依次更新 `RunLog` -> `Item主表` -> `Turn主表` -> `Stats` -> `ClickHouse` 状态为 `Processing`。
2.  **异常容错机制**:
    *   **若 MQ 发送失败**: 函数立即返回错误，DB 状态保持 `Queueing` 不变。调度器下一轮循环会再次扫描并重新分发。
    *   **若 MQ 发送成功但 DB 报错**: 消息已发出，Worker 会收到任务。此时调度器因报错而终止，DB 状态仍为 `Queueing`。
    *   **幂等保障**: 下一轮调度器会再次发送 MQ。由于 Worker 在执行前会抢占 `lock:expt:item:{RunID}:{ItemID}`，重复的消息会被第二个 Worker 直接丢弃，从而保证任务不重跑。
    *   **最终补救**: 无论 `Processing` 状态是否更新成功，Worker 跑完后都会回调更新为 `Logged`。调度器的收割环节（Archive）最终会将数据拉齐。
...
> [!TIP] 什么时候可以复用 `evaluationVersionId` 到 `ResultId` 的映射
> 例如，所有评估器都已经评估完毕，但是入库的时候网络抖动等问题。或者部分评估器评估完了，部分没有评估，那么已经评估了的就不会再次评估。

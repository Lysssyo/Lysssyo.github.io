...
> [!NOTE]
> 新增前如果在内存里预生成了 `item_id`，但最终触发了 `ON DUPLICATE KEY UPDATE`，数据库中的主键可能与内存中不同，需要进行“主键回填”。具体策略参考：[050-用RowsAffected精确推算Insert数以及批量Upsert主键回填的写后读策略](050-用RowsAffected精确推算Insert数以及批量Upsert主键回填的写后读策略.md)
...
- 性能优化：
    - 复合索引：`(dataset_id, del_vn, item_id)`。根据最左前缀匹(dataset_id, add_vn, del_vn)配原则，数据库会先锁定 `dataset_id`，然后通过 `del_vn = MaxVersionNum`（草稿的存活判定条件）精确定位到所有活着的草稿数据。将 `item_id` 放在索引最后，可以极大地加速基于 `item_id` 的 Cursor 分页排序，避免额外的 `Using filesort` 性能消耗。
...
- 优势：
	- 索引：`(dataset_id,version_id,item_id)`。查询的时候可以完全利用这个复合索引做等值查询。
...
- 复合索引：(dataset_id, add_vn, del_vn)。等值条件 `dataset_id` 必须放在第一位。紧接着放置 `add_vn`，数据库可以快速扫描出所有在目标版本及之前创建的数据。虽然 MySQL 在遇到第一个范围查询（`add_vn <= target_vn`）后，后续的索引列（`del_vn > target_vn`）可能无法完全走 B+ 树的二分查找，但利用 Index Condition Pushdown (ICP) 特性，依然可以在引擎层直接过滤掉不符合 `del_vn` 条件的数据，大幅减少回表次数。
